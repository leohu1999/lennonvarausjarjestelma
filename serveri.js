...
// Query-muuttujan avulla kyselyt voidaan tehdä sql:ään
const query = util.promisify(con.query).bind(con);
app.use(express.static(path.join(__dirname, 'public')));

//Päivämäärä tietojen luominen, lentojen varaukseen liittyviä muuttjia ja html-sivujen tuominen serverille muuttujiin.
var d = new Date();
...
app.get('/public/kohteet.html', function (req, response) {
    ...
    (async () => {
        try {
            ...
            Object.keys(rows).forEach(function (key) {
                var row = rows[key];
                response.write('<tr onclick="Hae(this)">');
                response.write('<td>' + row.destination_name+ '</td>');
                response.write('<td>' + row.country+ '</td>');
                response.write('</tr>');
        ...
        try {
            ...
            if (alkuperaisetPaikat > req.body.varausPaikat) {
                ...
                if ((seats[0] - (req.body.varausPaikat - alkuperaisetPaikat)) >= 0) {
                    ...
                    response.write('<p id="vahvistustekstiterror">Varauksen muokkaus epäonnistui! Koneeseen mahtuu vielä '+ seats+' henkilöä</p>');
                }
    ...
    (async () => {
        try {
            ...
            if ((seats - req.body.maara) >= 0) {
                ...
                response.write('<p id="vahvistustekstiterror">Varauksen vahvistus epäonnistui! Koneessa tilaa ' + seats[0]+' paikkaa</p>');
            }
...
app.get('/public/:id', function(req, response) {
    response.writeHead(200, {"Content-Type": "text/html"});
    response.write(kohteet);
    response.write('<table id="lennot"><tr>');
    ...
    console.log(quer);
    let sql = "SELECT destination_name, country, destination_id FROM destination WHERE destination_name='" + quer + "';";
    (async () => {
        try {
            ...
            Object.keys(rows2).forEach(function (key) {
                ...
                response.write('<td>' + date.getFullYear() + '-' + (date.getUTCMonth()+1) + '-' +(date.getUTCDate()+1)+ '</td>');
                response.write('<td>' + row.time + '</td>');
                response.write('<td>' + rows[0].destination_name + '</td>');
                response.write('<td>' + rows[0].country + '</td>');
                response.write('<td>' + row.seats + '</td>');
                response.write('</tr>');
            });
        response.end('</table>')
